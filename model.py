...
class NetArticle(nn.Module):
    ...
    def body(self, x):
        ...
        legacy1 = nn.ReLU(legacy1)
        legacy1 = self.conv_down_2(legacy1)
        legacy1 = nn.ReLU(legacy1)   # shape is 32
        legacy2 = self.conv_down_3(legacy1)
        legacy2 = nn.ReLU(legacy2)
        legacy2 = self.conv_down_4(legacy2)
        legacy2 = nn.ReLU(legacy2)   # shape is 64
        legacy3 = self.conv_down_5(legacy2)
        legacy3 = nn.ReLU(legacy3)
        legacy3 = self.conv_down_6(legacy3)
        legacy3 = nn.ReLU(legacy3)   # shape is 128 (actually we needn't legacy3)
